...
if __name__ == "__main__":
    ...
    for i, fname in enumerate(os.listdir(os.path.join(path, "ref"))):
        img = np.array(Image.open(os.path.join(path, "img", fname.split(".")[0] + ".png")))
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        # cv2.imshow(fname, img)
        # cv2.waitKey()
        # cv2.destroyAllWindows()
        ref = np.array(Image.open(os.path.join(path, "ref", fname)))
        ref = cv2.cvtColor(ref, cv2.COLOR_RGB2BGR)
        ref = cv2.resize(ref, (256, 256))
        # cv2.imshow(fname, ref)
        # cv2.waitKey()
        # cv2.destroyAllWindows()
        start = time.time()
        ...
        cv2.imwrite(os.path.join(SAVE_DIR, fname), result)
